...
export const getInstructorOfStudent = (studentId) => {
  const studentInstructors = students.find(
    (student) => student.id === studentId
  );

  return instructors.find(
    (instructor) => instructor.id === studentInstructors.instructorId
  );
};
...
export const getStudentWithMostLangs = () => {
  const lang = students.map((student) => student.languages.length);
  return students.find(
    (student) => student.languages.length === Math.max(...lang)
  );
};
