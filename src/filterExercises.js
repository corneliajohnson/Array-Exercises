...
export const getPolyglotStudents = (languageCount) => {
  return students.filter(
    (student) => student.languages.length >= languageCount
  );
};
...
export const getAvailableInstructors = () => {
  const unAvailable = students.map((student) => student.instructorId);

  return instructors.filter(
    (instructor) => unAvailable.includes(instructor.id) === false
  );
};
...
export const getStudentsByLanguages = (languages) => {
  ...
  languages.map((language) => {
    hasLanguages = students.filter((student) =>
